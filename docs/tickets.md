...
**対応履歴:**
- 2026-02-19: NDL SRU APIのXMLパース修正（`<srw:recordData>` 名前空間対応）および `recordPacking=string` 追加を実施したが、改善されず
- 引き続き未解決。追加調査が必要:
  - Google Books API が `maxResults=40` を指定しても実際に返す件数が少ない可能性（APIキーなし時の制限）
  - NDL APIのレスポンス内容を実際にログ出力して確認する
  - Google Books の複数ページ取得（`startIndex` パラメータによるページネーション）を検討
